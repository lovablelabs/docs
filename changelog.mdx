...
<Update label="10/06/2025">
 ...
 Build full‑stack apps from a single prompt, with no manual Supabase or API setup. Lovable Cloud gives you on‑demand secure hosting, compute, and storage that scale automatically as your app grows.  
Lovable Cloud is enabled on all workspaces by default and can be toggled on or off within new and existing projects. Learn more [here](https://docs.lovable.dev/features/cloud). 

**Lovable AI** 

        ![image](/images/lvbl-ai-logo.png) 

You can now build AI-native apps in Lovable — no API keys, no LLM billing headaches, no response-format debugging.

Lovable does it all automatically: we manage provider access, routing, and response normalization so you can focus on your product. Through **October 13, 2025**, we’re offering free embedded AI functionality in apps built on Lovable via our friends at **Gemini** (yes, that’s bananas\!)
...
Every Lovable plan includes a baseline of embedded AI features at no extra cost. Full pricing and limits can be found [here](https://docs.lovable.dev/features/cloud#usage-based-cloud-and-ai-pricing).
...
Drop any files into Lovable to turn them into an app. For example: 
...
Anthropic’s latest model now powers all code generation by default. Expect stronger reasoning, more consistent results, and better performance on multi‑step edits and structured data.

**A smoother build experience**  

We’re making building on Lovable even more, well, lovable with easier access to the Visual Editor, better UX on longer reasoning runs, and image support in the Code Editor.
