...
- Redesigned mobile experience for building apps and websites directly from your phone with an optimized interface.
- Visual Edits refresh with new floating combo box controls positioned right next to elements you're editing, featuring both natural language AI input** and **precision styling controls.
- Added project search functionality with fuzzy matching by name – _tip: rename your projects for better discoverability_.
- Rolled out full-stack initial generations to all users for more comprehensive app scaffolding.
- Floating prompt bar on homepage now lets you start prompting even when scrolled down.
- Enhanced history view with a cleaner timeline prioritizing dates, restored versions now visible, and simplified version navigation.
- Updated careers page with latest information. [Check it out here](https://lovable.dev/careers).
