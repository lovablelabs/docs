...
<Note>
  **Service provider (SP)-initiated sign-in**. Sign in starts from Lovable. IdP-initiated SSO (launching from your provider's dashboard tile) is not supported.
</Note>
...
To connect your identity provider to Lovable using OIDC or SAML, you require:
...
Lovable instructs you to set up your identity provider before proceeding and provides relevant setup information and settings to use in your identity provider. See the respective OIDC and SAML tabs below for a quick reference.
  ...
  <Tab title="OIDC">
    <Steps>
      ...
      <Step title="Save application">
        Click **Save**. Your application is now created.
      </Step>
      ...
      <Step title="(Optional) Test configuration">
        Click **Test configuration**. If everything is configured correctly, the validation should be successful.
      </Step>
...
<AccordionGroup>
  <Accordion title="I already have an account, but I'm joining a business workspace that uses SSO. How do I log in?">
    If you created your account using another login method (like email/password, Google, or GitHub), you need to log in that way **first**. Once you're logged in, navigate to Settings → Identity → Link with SSO. This will link your existing account to your SSO identity.
  ...
  <Accordion title="Can I enforce SSO for my workspace?">
    Yes. The workspace **owner**  or **admin** can enable **Enforce SSO** in **Settings → Workspace → Identity**.
