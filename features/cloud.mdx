...
By default, Cloud is enabled at workspace level. Only workspace owners and admins can enable or disable Cloud at the workspace level.

Cloud must be enabled at the workspace level before it can be used in any projects. If Cloud is disabled at the workspace level, the agent will notify you and suggest enabling it before continuing.

### User level

By default, enabling the Cloud integration is set to **Ask each time** on user level. This means Cloud will be enabled or disabled for your project based on your answer. You can change your personal preference from **Settings → Account → Tools.**
...
- **Adjust preferences**: set personal defaults for future use. Set to **Ask each time** by default.
