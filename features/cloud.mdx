...
By default, Lovable Cloud is enabled for your workspace. From your user preferences (**Settings → Account → Tools)**, you can choose the default behavior you as a user want in your projects.

### User preferences

By default, enabling the Cloud integration is set to **Ask each time** in your user preferences. This means you choose to enable or disable Cloud for your project each time. You can change your preference from **Settings → Account → Tools.**

Lovable prompts you when a feature requires backend functionality. For example, if authentication or file storage is needed, you can:
...
- **Adjust preferences**: change the default behavior, which affects future projects and not the current one. Set to **Ask each time** by default.

### Lovable Cloud tools - user preferences

You can configure Cloud integration tools from your user preferences by going to **Settings → Account → Tools**.
