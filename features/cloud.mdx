...
Lovable Cloud utilizes Supabase’s open-source foundation, which means you get a production-ready environment from day one, without needing to set up Supabase separately. You have the power of a production-ready PostgreSQL database, real-time updates, user auth, and storage — with none of the manual setup steps.
...
- **Fully portable**: Utilizes Supabase’s open-source stack. You retain full ownership of your app and data with the freedom to export, self-host, or migrate anytime.
- **Cost-effective**: Manage multiple projects in one workspace with usage-based pricing. Every workspace starts with **\$25 free usage**, enough to launch and run multiple projects. Read more in [Usage-based Cloud and AI pricing](https://docs.lovable.dev/features/cloud#usage-based-cloud-and-ai-pricing).

## Enabling Lovable Cloud

Lovable Cloud integration behavior can be managed at the workspace, user, and project level.

- Cloud must be enabled at the workspace level before it can be used in any projects.
- Only workspace owners and admins can enable or disable Cloud at the workspace level.

### Default behavior

| Lovable Cloud setting level | Default behavior |
| --------------------------- | ---------------- |
| Workspace                   | Enabled          |
| User                        | Ask each time    |

At the user level, Lovable prompts you whenever a feature requires backend functionality. For example, if authentication or file storage is needed, you can:
...
If Cloud is disabled at the workspace level, the agent will notify you and suggest enabling it before continuing.

## Lovable Cloud tools

You can configure Cloud integration tools from **Settings → Account → Tools** at either the workspace or project level.
...
| ------------------- | ------------------------------------------------------------------------------------------------------------------ | ---------------- |
| Enable integration  | Create and connect a Cloud project to add backend features (database, auth, storage). Required to use Cloud tools. | Ask each time    |
| Read queries        | Allow Lovable to read your database.                                                                               | Always allow     |
| Analytics queries   | Query logs to investigate issues and behavior.                                                                     | Always allow     |
| Security linter     | Run automated security checks on your database. Not a full audit.                                                  | Always allow     |
| Database migrations | Make schema and policy changes via SQL migrations. Updates generated types.                                        | Always allow     |
| Insert data         | Add data to your database tables.                                                                                  | Always allow     |
| Project info        | Read project configuration and environment details.                                                                | Always allow     |
...
You’ll find the **Cloud tab** inside any Lovable project by clicking the `+` button next to **Preview** and **Code mode**.

The following features live inside the Cloud tab:
...
- **Debugging built-in**: each function includes logs of executions, output, and errors. If something fails, Lovable surfaces the error directly in chat for quick troubleshooting.

### AI

- Monitor Lovable AI usage and performance for your project.
- For more information, see [Lovable AI documentation](https://docs.lovable.dev/features/ai)
...
<Note>
  Some secrets, such as the Lovable API key, are required for the platform and cannot be deleted.
</Note>
...
**Prompt:**
Add a contract upload field to each deal. Let clients have profile pictures and display them on the client detail page. Upload invoice PDFs to and show a download link on the invoices table.
...
**Prompt:**
When a new lead is added to the Clients table, send me an email notification. Analyze client meeting notes with Lovable AI and save a sentiment score in the Clients table. Sync new contacts automatically. Schedule a daily job at 7am that emails me a summary of today’s follow-ups. When a payment is marked ‘paid’ in Stripe, automatically update the related invoice’s status to ‘Paid’.
...
Every workspace receives free monthly usage, regardless of plan:

- **\$25 Cloud balance** for app hosting
- **\$1 AI balance** for AI features

These free funds reset on the first day of each month at 00:00 UTC (displayed in your timezone). Unused free funds do not roll over to the next month.

<Note>
  **Temporary Promotion:** Until the end of the year, all users get \$25 Cloud and \$1 AI per month, even on the Free plan.
</Note>
