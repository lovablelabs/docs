...
| **Verifying**        | Waiting for DNS records to propagate.                                                                                                                                                                                                             | Click **Check status** to check the latest status and verify the DNS records.<br /><br />Check that your `A` and `TXT` records are added correctly, especially the full TXT value. |
| **Unable to verify** | DNS verification did not complete within one hour. This usually means your DNS records are incorrect or still propagating. <br /><br />**Note**: _DNS propagation can take up to 72 hours, although most updates are applied within a few hours._ | Click **Check again** to refresh the status and re-verify your DNS records.<br /><br />Check that your `A` and `TXT` records are added correctly, especially the full TXT value.   |
