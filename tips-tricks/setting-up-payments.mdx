...
* The project 
  ...
  **must**
  ...
  [Learn more about Supabase](https://docs.lovable.dev/integrations/supabase)
...
* Retrieve the 
  ...
  **endpoint URL**
   ...
   from the Edge Function that you have set up.
...
* Navigate to 
  ...
  **Developers → Webhooks → Create an event destination**
...
* Add the webhook secret to Supabase as a secret value 

  **(Supabase → Edge Functions → Manage secrets → Add new secret)**

  . Ask the AI in chat mode what the secret should be called to work properly with the Edge function.
