...
<Accordion title="How to implement and verify canonical tags">
  ...
  **Verify**
  ...
  - Check each page. Paste this in browser console - should return 
...
Meta descriptions appear in search results just below the page title. They aren’t a direct ranking factor, but they have a strong influence on click-through rates. Lovable Agent can add meta descriptions when you request them. 
...
A clear heading structure (H1 to H3) helps search engines understand how your content is organized and which sections are most important. Lovable Agent can generate proper H1 tags and a logical hierarchy when you request it.
...
Semantic HTML uses meaningful tags to describe your content, helping both search engines and screen readers understand the structure of your page. Lovable supports semantic elements such as `<main>`, `<nav>`, and `<section>`.
...
Structured data, implemented through JSON-LD schema, gives search engines additional context about your content and can enable rich results like star ratings, product details, and FAQ dropdowns. Lovable can generate schema markup when asked.
...
<Accordion title="How to implement and verify structured data">
  ...
  **Verify**
  ...
  - Check presence on key pages (for example, home, products, blog posts, FAQ). Paste this in browser console - should return 1:
...
Lovable includes basic OG and Twitter metadata by default, but **you need to customize the image, title, and description for each page.**
...
<Accordion title="How to implement and verify Open Graph, Twitter Cards, and OG images">
  **Prompt to add**
  ...
  > Update social metadata for all key pages. Replace the default Lovable Open Graph and Twitter tags with unique values per route. For each page, set a custom `og:title`, `og:description`, `og:url`, and `twitter:card` (`summary_large_image`).
  >
  > Create a custom OG image for each page (1200×630 JPG, under 800KB) that includes the page title over branded visuals.
  >
  > Then update each page’s `og:image` and `twitter:image` to point to its specific file.
...
<Accordion title="How to set up and use GCS for monitoring and maintenance">
  ...
  **Use URL Inspection tool** (most powerful feature)
  ...
  - Check if important resources (JavaScript/CSS) are blocked
...
<AccordionGroup>
  ...
  <Accordion title="Can AI search engines (Perplexity, ChatGPT, Gemini, Claude) index Lovable apps?">
    ...
    The following helps AI crawlers extract facts for their knowledge graphs even without JavaScript execution:
  ...
  <Accordion title="What is prerendering, and when do I need it?">
    Prerendering/dynamic rendering gives bots a static HTML snapshot while users get the JavaScript app. It’s particularly helpful for:
  ...
  <Accordion title="What are the main CSR SEO limitations, and how do I work around them?">
    ...
    - Social previews don’t run JavaScript\
